...
func TestPlugin_Test_ConnectionError(t *testing.T) {
	...
	result := p.Test(ctx, "invalid-host:389", "admin", "password", 2*time.Second, brutus.PluginConfig{})
...
func TestPlugin_Test_Timeout(t *testing.T) {
	...
	result := p.Test(ctx, "192.0.2.1:389", "admin", "password", 1*time.Second, brutus.PluginConfig{})
