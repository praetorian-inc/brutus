...
func TestPlugin_Test_ConnectionError(t *testing.T) {
	...
	result := p.Test(ctx, "invalid-host:3306", "root", "password", 2*time.Second, brutus.PluginConfig{})

	assert.NotNil(t, result)
	assert.Equal(t, "mysql", result.Protocol)
	assert.Equal(t, "invalid-host:3306", result.Target)
	assert.False(t, result.Success)
...
func TestPlugin_Test_Timeout(t *testing.T) {
	...
	result := p.Test(ctx, "192.0.2.1:3306", "root", "password", 1*time.Second, brutus.PluginConfig{})
